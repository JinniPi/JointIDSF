...
class JointIDSF(RobertaPreTrainedModel):
    """
    JointIDSF model follow research "Intent Detection and Slot Filling for Vietnamese"
    """
    def __init__(self, config, args, intent_label_lst, slot_label_lst):
        super(JointIDSF, self).__init__(config)
        self.args = args
        self.num_intent_labels = len(intent_label_lst)
        self.num_slot_labels = len(slot_label_lst)

        # choose pretrain BERT or XLM
        if args.case == 'BERT':
            self.roberta = RobertaModel(config)
        else:
            self.roberta = XLMRobertaModel(config)

        # For intent detection part
        self.intent_classifier = IntentClassifier(config.hidden_size, self.num_intent_labels, args.dropout_rate)

        # For slot filling part
